...
function fixFile(filePath) {
  ...
  const singleQuotePattern = "'\\${API_BASE}'";
  if (content.includes(singleQuotePattern)) {
    content = content.split(singleQuotePattern).join("`\\${API_BASE}`");
    changed = true;
  }

  // إصلاح: "${API_BASE}" → `${API_BASE}`
  const doubleQuotePattern = '"\\${API_BASE}"';
  if (content.includes(doubleQuotePattern)) {
    content = content.split(doubleQuotePattern).join("`\\${API_BASE}`");
    changed = true;
  }
