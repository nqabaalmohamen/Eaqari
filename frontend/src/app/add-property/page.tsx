...
export default function AddProperty() {
  ...
  useEffect(() => {
    ...
    if (editParam) {
      ...
      fetch(`https://eaqari.vercel.app/api/properties/${id}`)
        .then(r => r.json())
        .then(p => {
  ...
  const handleOfferSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Upload new images as base64
      const imageUrls: string[] = [];
      for (const file of offerImages) {
        const reader = new FileReader();
        await new Promise<void>((resolve) => {
          reader.onload = (ev) => {
            if (ev.target?.result) imageUrls.push(ev.target.result as string);
            resolve();
          };
      ...
      const url = isEditMode
        ? `https://eaqari.vercel.app/api/properties/${editId}`
        : 'https://eaqari.vercel.app/api/properties';
