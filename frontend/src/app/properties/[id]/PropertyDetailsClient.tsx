...
export default function PropertyDetailsClient({ id }: { id: string }) {
    ...
    const fetchProperty = async () => {
      try {
        ...
        const formatted: Property = {
          ...
          location: String(p.region || '') + '، ' + String(p.city || ''),
          district: p.city,
          ...
          ownerId: owner_id,
          features: [
            ...(feats.has_elevator ? ['مصعد'] : []),
            ...(feats.has_parking ? ['موقف سيارات'] : []),
            ...(feats.is_licensed ? ['مرخص'] : []),
            ...(feats.has_electricity ? ['عداد كهرباء'] : []),
            ...(feats.has_water ? ['عداد مياه'] : []),
            ...(feats.has_gas ? ['عداد غاز'] : []),
          ],
          floor: p.floor || 'غير محدد',
          finishing: p.finishing_type || 'غير محدد',
      ...
      <div className="min-h-[70vh] flex flex-col items-center justify-center gap-4 p-6 text-center" dir="rtl">
        ...
        <div className="flex flex-col sm:flex-row gap-2 pt-2">
          <button
            ...
            className="bg-blue-600 text-white font-bold text-sm px-6 py-3 rounded-2xl hover:bg-blue-700 transition-colors"
          >
            🏠 عرض جميع العقارات
          </button>
          <button
            onClick={() => router.back()}
            className="bg-gray-100 text-gray-700 font-bold text-sm px-6 py-3 rounded-2xl hover:bg-gray-200 transition-colors"
          >
            رجوع
          </button>
        </div>
  ...
  if (loadState === 'hidden') {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center gap-4 p-6 text-center" dir="rtl">
        ...
        <p className="text-gray-500 text-sm max-w-xs">
          هذا الإعلان قيد المراجعة أو تم رفضه من قبل فريق الإدارة، لذلك لا يظهر للعامة حالياً.
        </p>
        <div className="flex flex-col sm:flex-row gap-2 pt-2">
          <button
            onClick={() => router.push('/properties')}
  ...
  return (
    <div className="space-y-4 pb-24" dir="rtl">
      ...
      {favMsg && (
        <div className="bg-green-50 border border-green-200 text-green-700 text-xs font-bold px-4 py-2.5 rounded-xl text-center">{favMsg}</div>
      )}
      ...
      {isOwner && property.status === 'rejected' && (
        <div className="bg-red-50 border-2 border-red-200 rounded-3xl p-5 space-y-2 shadow-sm">
          <div className="flex items-center gap-2.5">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-red-500 font-medium">
                هذا الإعلان غير منشور للعامة — لا يظهر لأي مستخدم آخر.
              </p>
            </div>
          </div>
          <div className="bg-white rounded-2xl border border-red-100 p-4 space-y-1.5 mt-2">
            ...
            <p className="text-sm text-red-800 leading-loose pr-1">
              {property.rejection_reason || 'لا يوجد سبب مذكور — يمكنك التواصل مع الإدارة لمعرفة التفاصيل.'}
            </p>
          </div>
          <div className="flex flex-wrap gap-2 pt-1">
            <button
              onClick={() => router.push(`/add-property?edit=${property.id}`)}
              className="flex-1 bg-blue-600 text-white font-bold text-sm py-3 rounded-2xl hover:bg-blue-700 transition-colors"
            >
              ✏️ تعديل الإعادة وأعادة النشر
            </button>
            <button
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-yellow-600 font-medium">
                هذا الإعلان لا يظهر للعامة حالياً — ينتظر مراجعة فريق الإدارة قبل النشر.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="relative h-56 w-full rounded-3xl overflow-hidden shadow-sm bg-gray-100">
        ...
        <div className="absolute top-3 right-3 flex gap-2">
          ...
          {property.verified && (
            <span className="bg-green-600/90 text-white text-[10px] font-bold px-3 py-1.5 rounded-full">موثق</span>
          )}
          {isOwner && property.status && property.status !== 'active' && (
      ...
      <div className="grid grid-cols-2 gap-3">
        {[
          { label: 'المساحة', value: property.area, icon: '📐' },
          { label: 'الغرف', value: String(property.rooms) + ' غرفة', icon: '🛏' },
          { label: 'الحمامات', value: String(property.baths) + ' حمام', icon: '🚿' },
          { label: 'المطابخ', value: String(property.kitchens) + ' مطبخ', icon: '🍳' },
        ].map((spec) => (
          <div key={spec.label} className="bg-white p-3 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-2">
            <span className="text-xl">{spec.icon}</span>
            <div>
              <span className="text-gray-400 text-[10px] font-medium block">{spec.label}</span>
              <span className="text-gray-700 font-black text-sm">{spec.value}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm space-y-3">
        <h3 className="font-bold text-sm text-gray-800 border-b border-gray-100 pb-2">تفاصيل إضافية</h3>
        <div className="grid grid-cols-2 gap-y-3 text-xs">
          <div><span className="text-gray-400 font-medium block">الدور</span><span className="text-gray-700 font-bold">{property.floor}</span></div>
          <div><span className="text-gray-400 font-medium block">التشطيب</span><span className="text-gray-700 font-bold">{property.finishing}</span></div>
      ...
      <div className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm space-y-3">
        <h3 className="font-bold text-sm text-gray-800 border-b border-gray-100 pb-2">الوصف الكامل</h3>
        <p className="text-sm text-gray-600 leading-loose">{property.description}</p>
      </div>

      {property.features && property.features.length > 0 && (
        <div className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm space-y-3">
          <h3 className="font-bold text-sm text-gray-800 border-b border-gray-100 pb-2">المميزات والمرافق</h3>
      ...
      {/* Hide contact/chat/report for non-owners when not active */}
          ...
          {!isOwner ? (
            <div className="grid grid-cols-2 gap-2">
              ...
              <button onClick={handleStartChat}
                className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-2xl transition-colors text-sm">
                💬 محادثة
              </button>
            </div>
          ) : (
            <div className="grid grid-cols-2 gap-2">
              <button
                onClick={() => router.push(`/add-property?edit=${property.id}`)}
                className="bg-blue-500 hover:bg-blue-600 text-white font-bold py-3 rounded-2xl transition-colors text-sm"
              >
                ✏️ تعديل
              </button>
              <button
                onClick={() => router.push('/my-ads')}
                className="bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-3 rounded-2xl transition-colors text-sm"
              >
                إعلاناتي
              </button>
            </div>
      ...
      {!isOwner && (property.status === 'active' || property.status === 'sold' || property.status === 'rented') && (
        <button onClick={() => setShowReportModal(true)}
          className="w-full text-xs text-gray-400 font-medium py-2 text-center hover:text-red-500 transition-colors">
      ...
      {/* Login Modal */}
      {showLoginModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[60] p-4" onClick={(e) => { if (e.target === e.currentTarget) setShowLoginModal(false); }}>
          <div className="bg-white rounded-3xl p-6 max-w-sm w-full text-center space-y-4" dir="rtl">
            ...
            <div className="pt-2 flex flex-col gap-2">
              <button onClick={() => router.push('/login')} className="w-full bg-blue-600 text-white font-bold py-3 rounded-2xl hover:bg-blue-700 transition-colors">
                تسجيل الدخول / حساب جديد
              </button>
              <button onClick={() => setShowLoginModal(false)} className="w-full bg-gray-50 text-gray-600 font-bold py-3 rounded-2xl hover:bg-gray-100 transition-colors">
                إلغاء
              </button>
            </div>
          </div>
